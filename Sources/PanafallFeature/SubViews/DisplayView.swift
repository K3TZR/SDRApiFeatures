  ...
  var body: some View {
    ...
    VStack(alignment: .leading, spacing: 8) {
      HStack(spacing: 10) {
        ...
        Slider(value: Binding(get: { Double(store.panadapter.average) }, set: { store.panadapter.setProperty(.average, String(Int($0))) }), in: 0...100, step: 1)
      }
      HStack(spacing: 10) {
        Text("Frames/sec").frame(width: 90, alignment: .leading)
        Text("\(store.panadapter.fps)").frame(width: 25, alignment: .trailing)
        Slider(value: Binding(get: { Double(store.panadapter.fps) }, set: { store.panadapter.setProperty(.fps, String(Int($0))) }), in: 0...100, step: 1)
      }
      HStack(spacing: 10) {
        ...
        Slider(value: $store.spectrumFillLevel, in: 0...100, step: 1)
//        Slider(value: viewStore.binding(get: {_ in Double(panadapter.fillLevel) }, send: { .panadapterProperty(panadapter, .fillLevel, String(Int($0))) }), in: 0...100, step: 1)
      }
      HStack {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      HStack(spacing: 10) {
        ...
        Slider(value: Binding(get: { Double(store.waterfall.colorGain) }, set: { store.waterfall.setProperty(.colorGain, String(Int($0))) }), in: 0...100, step: 1)
      }
      HStack(spacing: 10) {
        ...
        Slider(value: Binding(get: { Double(store.waterfall.blackLevel) }, set: { store.waterfall.setProperty(.blackLevel, String(Int($0))) }), in: 0...100, step: 1)
      }
      HStack(spacing: 10) {
        Text("Line Duration").frame(width: 90, alignment: .leading)
        Text("\(store.waterfall.lineDuration)").frame(width: 25, alignment: .trailing)
        Slider(value: Binding(get: { Double(store.waterfall.lineDuration) }, set: { store.waterfall.setProperty(.lineDuration, String(Int($0))) }), in: 0...100, step: 1)
      }
    }
  }
}
